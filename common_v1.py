...
def do_inference(context, bindings, inputs, outputs, stream, batch_size=10):
...
def do_inference_v2(context, bindings, inputs, outputs, stream):
    ...
    time2 = time.time()
    [cuda.memcpy_dtoh_async(out.host, out.device, stream) for out in outputs]
    time_out_GPU = time.time() - time2
    # Synchronize the stream
    time1 = time.time()
    stream.synchronize()
    time_synch = time.time() - time1
    # Return only the host outputs.
    return [out.host for out in outputs], time_in_GPU, infer_time, time_out_GPU, time_synch
